(function() {
  'use strict';
  angular.module('featureToggle', ['ui.router', 'semver'])
...
///////////////
// config
  function overrideUIRouterStateFn($stateProvider) {
    var oldStateFn = $stateProvider.state;
    $stateProvider.state = function(name, conf, feature) {
      // enable state if feature is true
      if ((feature === true) || (typeof feature === 'undefined')) {
        try {
          return oldStateFn.call($stateProvider, name, conf);
        }
        catch(e) {
          window.console && console.warn('state ' + name + ' is already defined');
